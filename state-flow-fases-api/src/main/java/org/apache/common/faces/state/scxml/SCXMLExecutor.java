...
import org.apache.common.faces.state.scxml.model.Observable;
import org.apache.common.faces.state.scxml.model.SCXML;
import org.apache.common.faces.state.scxml.model.TransitionTarget;
import org.apache.common.faces.state.scxml.semantics.SCXMLSemanticsImpl;
 ...
 * SCXMLSemantics implementation that it uses.</p>
 *
 * <p>
 ...
 *
 * <p>
 * The executor uses SCXMLExecutionContext to manage the state and provide all
 * the services to the SCXMLSemantics implementation.</p>
 *
 * @see SCXMLSemantics
 */
public final class SCXMLExecutor implements SCXMLIOProcessor, StateHolder {
     ...
     * @param invokeId
     * @param scxml
     * @throws org.apache.common.faces.state.scxml.model.ModelException
     */
    ...
    /**
     * starts the state machine with a specific active configuration, as the
     * result of a (first) step
     * <p>
     * This will first (re)initialize the current state machine: clearing all
     * variable contexts, histories and current status, and clones the SCXML
     * root datamodel into the root context.
     * </p>
     *
     ...
     * configured yet, when an unknown or illegal stateId is specified, or when
     * the specified active configuration does not represent a legal
     * configuration.
     ...
     * <p>
     * The root context can be used for providing external data to the state
     * machine
     * </p>
     *
     ...
     * itself and should be regarded and treated "read-only" from external
     * usage.
     * </p>
     *
     ...
     * and should not be altered once in use. Also note that manipulation of
     * instance data for the executor should happen through its root context or
     * state contexts only, never through the direct manipulation of any
     * {@link org.apache.commons.scxml2.model.Datamodel}s associated with this
     ...
     * </p>
     * <p>
     * Note: until an instance is re-attached, no operations are allowed (and
     * probably throw exceptions) except for {@link #addEvent(TriggerEvent)}
    ...
    /**
     * Clear all state, optionally initialize/override global context data, and
     * begin executing the state machine
     *
     * @param data
     * @data optional data to initialize/override data defined (only) in the
     * global context of the state machine
    ...
    /**
     * Add a new external event, which may be done concurrently, and even when
     * the current SCInstance is detached.
     * <p>
    ...
    /**
     * Trigger all pending and incoming events, until there are no more pending
     * events
     *
     * @throws ModelException in case there is a fatal SCXML object model
     * problem.
